...
class MonacoConvergenceAdapter {
  ...
  shareData(){
    ...
    this.contentManager = new MonacoCollabExt.EditorContentManager({
      ...
      onDelete:(index,length)=>{ //delete text at given index
        this._model.remove(index,length);
      },
      remoteSourceId:"convergence",
    });

    this._model.events().subscribe(e=>{ //subscribe event of models emitted
      switch(e.name){
        case"insert":{
  ...
  shareRemoteCursor(){
    ...
    this._editor.onDidChangeCursorPosition(e=>{//listen to event emitted when loacl cursor changes the position
      this.setLocalCursor();
    });
  ...
  shareSelection(){
    ...
    this._editor.onDidChangeCursorSelection(e=>{ //listen to event emitted when selection change
      this.setLocalSelection();
    });
    ...
    this._model.on("reference",e=>{ //reference event is fired when a new reference is created , so we listen and add the  selection for it if exist
...
/////////// Helper function for Selection Binding ///////////////
  setLocalSelection(){
    ...
    if(!selection.isEmpty()){
      ...
      .getModel().getOffsetAt(selection.getStartPosition());
      const end = this._editor
      .getModel().getOffsetAt(selection.getEndPosition());
      this.selectionReference.set({start,end});
    }else if(this.selectionReference.isSet()){ //if Selection is already set in  editor
...
//////////////// Helper functions for Cursor Binding //////////////
  ...
  addRemoteCursor(reference){
    ...
    reference.on("set",()=>{//when remote cursor is set
